...
**현재 버전**: 1.6.0 (시세그래프/무기해방/스카우터 버그픽스)
...
## 최근 업데이트 (2025-07, v1.6.0)

- **아이템 시세 그래프**
  - 모달이 화면 80% 크기로 확장, 차트가 남는 영역을 모두 채움
  - Chart.js 옵션 및 flex 레이아웃 개선으로 차트가 항상 꽉 차게 표시
...
- **UI/UX**
  - 모달/차트가 항상 보이도록 flex, min-height, min-width 등 보강
  - 버튼/입력창/닫기 등도 시각적으로 안정적으로 유지 