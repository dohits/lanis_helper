...
### Vite 마이그레이션 완료
- 빌드 시스템을 Vite로 전환하여 성능 향상
- ES6 모듈 시스템 도입으로 코드 구조 개선
- 번들 크기 최적화 (content script: 85.49 kB)

### 아이템 스카우터 문제 해결
- CSS 선택자 유연성 향상 (`[class*="css-"]` 패턴 사용)
- 초기화 순서 최적화 (아이템 스카우터 우선 초기화)
- 매니페스트 업데이트 스크립트 수정
- `ITEM_COLORS` 모듈 import 누락 문제 해결
- 디버깅 로그 추가로 문제 추적 개선

---

**마지막 업데이트**: 2025년 7월 23일 (v1.5.2) 