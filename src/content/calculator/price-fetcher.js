...
class PriceFetcher {
  ...
  async fetchData() {
    try {
      ...
      const oldSheetUrl = `https://docs.google.com/spreadsheets/d/${this.sheetId}/gviz/tq?tqx=out:csv&gid=${this.oldGid}`;
      ...
      const newSheetUrl = `https://docs.google.com/spreadsheets/d/${this.sheetId}/gviz/tq?tqx=out:csv&gid=${this.newGid}`;
      const newRes = await fetch(newSheetUrl);
      if (!newRes.ok) throw new Error('새로운 시트 데이터 요청 실패');
      const newCsv = await newRes.text();
      
      // 두 시트 데이터 파싱
      const oldRows = this.parseCSV(oldCsv);
      const newRows = this.parseCSV(newCsv);
  ...
  async getItemPrice(itemName, priceType = 'recent') {
    try {
      const { oldRows, newRows } = await this.fetchData();
      
      if (oldRows.length < 2 && newRows.length < 2) {
